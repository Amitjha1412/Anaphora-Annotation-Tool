...
public class AnaphoraAnnotation extends JFrame implements ActionListener {
    ...
    public static void main(String[] args) {
        new AnaphoraAnnotation();
    }

    /**
     * Constructor for the AnaphoraAnnotation main window
     *
     **/
    public AnaphoraAnnotation() {
        setVisible(true);
        setTitle("AnaphoraAnnotation");
    ...
    /**
     * Creates the menubar of AnaphoraAnnotation
     *
     **/
    private void createMenuBar() {
        ...
        menubar.add(menu);
        setJMenuBar(menubar);
        menu = new JMenu("Annotate");
        menu.setMnemonic('a');
        ArrayList<MenuDetail> annotate = new ArrayList<MenuDetail>();
        annotate.add(new MenuDetail("Mark Pronoun...", "pronoun", 'g', null, false));
        annotate.add(new MenuDetail("Mark Noun...", "noun", 'j', null, false));
        //  annotate.add(new MenuDetail("Annotate...", "annotate", 'q', null, false));
        for (MenuDetail m : annotate) {
            mi = new JMenuItem(m.menuName, m.ac);
            mi.setAccelerator(m.keystroke);
            mi.setActionCommand(m.actionLabel);
            mi.addActionListener(this);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if("pronoun".equals(cmd)) {
            showHighlightPronoun();
        }
        if("noun".equals(cmd)) {
            showHighlightNoun();
        }
                ...
                else
                {
                    ...
                    while((c=r.read())!=-1){
                        if(c=='\t' && tabcount==0)
                        {
                            tabcount++;
                            String adder="";
                            while((c=r.read())!='\t' && c!=-1 && c!='\n'){
                                adder+=(char)c;
                            }
                            if(c=='\n')
                                tabcount=0;
                            if((adder.indexOf("(("))==-1 && (adder.indexOf("))")==-1))
                            {
                                fileContent+=adder;
                                fileContent+="  ";
                            }
    ...
    private void showHighlightPronoun() {
        Highlighter h = textAreaContent.get(tabbedPane.getSelectedIndex()).getTextArea().getHighlighter();
        h.removeAllHighlights();
        try {
            pronounIndex = textAreaContent.get(tabbedPane.getSelectedIndex())
                .getTextArea().getSelectionStart();
            int e = textAreaContent.get(tabbedPane.getSelectedIndex())
                .getTextArea().getSelectionEnd();
            h.addHighlight(pronounIndex,e,DefaultHighlighter.DefaultPainter);
        }
        catch(Exception ble){
        }
    }
    private void showHighlightNoun() {
        Highlighter h = textAreaContent.get(tabbedPane.getSelectedIndex()).getTextArea().getHighlighter();
        //h.removeAllHighlights();
        try {
            int flag=0;
            String nounName="",type="";
            int sentenceIdNoun=1,sentenceIdPronoun=1,sentenceId=1;
            nounIndex = textAreaContent.get(tabbedPane.getSelectedIndex())
                .getTextArea().getSelectionStart();
            int e = textAreaContent.get(tabbedPane.getSelectedIndex())
                .getTextArea().getSelectionEnd();
            h.addHighlight(nounIndex,e,DefaultHighlighter.DefaultPainter);
            myFile = new File(textAreaContent.get(tabbedPane.getSelectedIndex()).getFileDir());
            ...
            while((c=r.read())!=-1){
                ...
                bufferedwriter+=(char)c;
                if(c=='\t' && tabcount==0){
                    if(totalCount == nounIndex) {
                        //flag=1;
                        sentenceIdNoun=sentenceId;
                        nounName = "" ;
                        nounName+=type;
                    }
                    if(totalCount == pronounIndex) {
                        flag=1;
                        sentenceIdPronoun=sentenceId;
                     //   System.out.println(sentenceIdPronoun);
                    }
                    tabcount++;
                    String adder="";
                    String searchref="";
                    ...
                    if(c=='\n')
                        tabcount=0;
                    if((adder.indexOf("(("))==-1 && (adder.indexOf("))"))==-1){
                        totalCount+=adder.length()+2;
                    }
                    else if(adder.indexOf("((")!=-1){
                        while((c=r.read())!='\''){
                            bufferedwriter+=(char)c;
                        }
                        bufferedwriter+=(char)c;
                    ...
                    if(flag==1){
                    	System.out.println("amit jha");
                        while((c=r.read())!='\n'){
                        	bufferedwriter+=(char)c;
                            if(c == ' ')
                                searchref= "" ;
                            searchref+=(char)c;
                         //   System.out.println(searchref);
                            if(searchref.indexOf("ref")!=-1){
                            	System.out.println("amit jha1");
                                c=r.read();
                                bufferedwriter+=(char)c;
                                c=r.read();
                                bufferedwriter+=(char)c;
                                if(sentenceIdPronoun!=sentenceIdNoun){
                                    bufferedwriter+='.';
                                    bufferedwriter+='.';
                                    bufferedwriter+=sentenceIdNoun;
                                    bufferedwriter+='$';
                                    System.out.println(bufferedwriter);
                                }
                                bufferedwriter+=nounName;
                                while((c=r.read())!='\'');
                                bufferedwriter+=(char)c;
                                while((c=r.read())!='\n')
                                	bufferedwriter+=(char)c;
            ...
            bufferedwriter+=(char)c;
            //System.out.printf(bufferedwriter);
            r.close();
            String filePath = textAreaContent.get(tabbedPane.getSelectedIndex()).getFileDir();
            BufferedWriter bufferedWriter = null;
            try {
                bufferedWriter = new BufferedWriter(new FileWriter(filePath,
                            false));
